! This code is part of RRTM for GCM Applications - Parallel - Neural Networks (RRTMGP-NN)
! Supports Neural Networks for accelerated gas optics 
! Originally written by Robert Pincus and Eli Mlawer, but modified for ecRAD and RRTMGP-NN by Peter Ukkonen 
!!
!
...
module mo_gas_optics_rrtmgp
  ...
  use mo_gas_optics_kernels, only: interpolation,                                         &
                                   ...
                                   combine_2str_opt, compute_Planck_source, compute_Planck_source_nn, &
  ...
  use mod_mlp_rrtmgp,    only: rrtmgp_mlp_type
  implicit none
  private
  ...
  type, public :: ty_gas_optics_rrtmgp
    private
    integer,  dimension(:,:), allocatable,public :: band2gpt       ! (begin g-point, end g-point) = band2gpt(2,band)
    integer,  dimension(:),   allocatable,public :: gpt2band       ! band = gpt2band(g-point)
  ...
  contains
    ...
    ! Public procedures
    ! public interface
    ! ------- Procedures copied from mo_optical_props START ---------
    ...
    ! ------- Procedures copied from mo_optical_props END ---------
...
contains
  ...
  pure function is_initialized(this)
    ...
    is_initialized = allocated(this%band2gpt)
  ...
  function init_base_from_copy(this, spectral_desc) result(err_message)
    class(ty_gas_optics_rrtmgp),    intent(inout) :: this
    class(ty_gas_optics_rrtmgp),    intent(in   ) :: spectral_desc
  character(len = 128)                        :: err_message
  ...
  function gas_optics_int_ecrad(this, ncol,nlay, ngpt,       &
    ...
    ! Optional inputs
    ! neural network models (uses NN kernel if present)
    type(rrtmgp_mlp_type), dimension(:), intent(in), &
          optional  :: neural_nets ! Emission and absorption models 
    ! ----------------------------------------------------------                              
    ! Local variables
    real(sp), dimension(:,:,:), allocatable           :: nn_inputs
    real(wp), dimension(:,:,:), allocatable           :: pfrac
    real(wp), dimension(size(play,dim=1), size(play,dim=2)) :: col_dry_arr
    ! ----------------------------------------------------------

    integer :: nband, ninputs, idx_h2o
    ! ----------------------------------------------------------
    ...
    if(check_values) then
      if(any_vals_outside(tlev, this%temp_ref_min,  this%temp_ref_max)) then 
        write(error_msg,*) &
          'rrtmgp od_lw tlev with min ', minval(tlev), ' and max ', maxval(tlev), 'was outside LUT range'
      end if
    ...
    if (present(neural_nets)) then
      ...
      call predict_nn_lw_blas(              &
              ...
              nn_inputs, col_dry_arr,        &  ! data inputs
              neural_nets, tau, pfrac)       ! outputs
#ifdef USE_TIMING
    ret =  gptlstart('compute_Planck_source_nn')
#endif
      call compute_Planck_source_nn(ncol, nlay, nband, ngpt,      &
        ...
        sfc_source, lev_source, pfrac)
      !$acc exit data delete(pfrac, nn_inputs)
      deallocate(pfrac, nn_inputs)
#ifdef USE_TIMING
    ret =  gptlstop('compute_Planck_source_nn')
#endif
    else
      ! Use interpolation routine, NOT neural networks for gas optics
      error_msg = compute_gas_optics_ecrad(this,                         &
                                   ncol, nlay, ngpt, nband,                 &
                                   play, plev, tlay, gas_desc, col_dry_arr,  &
                                   tau=tau,                                     &
    ...
    !$acc exit data delete(col_dry_arr, tlay, tlev, tsfc, plev, play)
  ...
  function gas_optics_ext_ecrad(this, ncol, nlay, ngpt,                        &
                          ...
                          neural_nets) result(error_msg)      ! optional input
    ...
    type(rrtmgp_mlp_type), dimension(2), intent(in), &
       optional      :: neural_nets ! Absorption model, Rayleigh model     
    ! ----------------------------------------------------------
    ! Local variables
    ...
    integer :: nband, ngas, ninputs
    integer :: idx_h2o!, igpt, icol
    ! ----------------------------------------------------------
    nband = this%get_nband()
    ngas  = this%get_ngas()
    ...
    if(check_values) then
      if(any_vals_outside(play, this%press_ref_min,this%press_ref_max)) then
        error_msg = "gas_optics(): array play has values outside range"
      end if
      if(any_vals_outside(tlay, this%temp_ref_min,  this%temp_ref_max)) then 
        write(error_msg,*) &
          'rrtmgp od_lw tlay with min ', minval(tlay), ' and max ', maxval(tlay), 'was outside LUT range'
      end if
    end if
    if(error_msg  /= '') return
    ...
    !    end do
    ! end do
    !$acc exit data delete(col_dry_arr, tlay, play, plev) 
  ...
  function compute_nn_inputs(ncol, nlay, ninputs,       &
    ...
    real(wp), dimension(nlay,ncol),           intent(in   ) ::  play, &   ! layer pressures [Pa, mb]; (nlay,ncol)
                                                                tlay
    type(ty_gas_concs),                       intent(in   ) ::  gas_desc  ! Gas volume mixing ratios  
    type(rrtmgp_mlp_type), dimension(:),  intent(in)    :: neural_nets ! needed to match inputs, load coefficients
    ...
    !     error_msg = "rrtmgp: gas",input_names(igas), 
    !  "was outside the range of the NN training data: <=", minval, " or >" maxval
    !     exit
    ! end do
    ! if(error_msg  /= '') return
    ...
    do icol = 1, ncol
      do ilay = 1, nlay
          ...
          nn_inputs(3,ilay,icol)    = ( sqrt(sqrt(gas_desc%concs(idx_h2o)%conc(ilay,icol))) - input_minvals(3)) / &
              & (input_maxvals(3) - input_minvals(3))
          nn_inputs(4,ilay,icol)    = ( sqrt(sqrt(gas_desc%concs(idx_o3) %conc(ilay,icol))) - input_minvals(4)) / &
              & (input_maxvals(4) - input_minvals(4))
      end do
    end do
    ...
    do igas = 5, ninputs
      ...
      if (error_msg == '') then
        if (ndims == 0) then
          ...
          do icol = 1, ncol
            do ilay = 1, nlay
                nn_inputs(igas,ilay,icol)    =  (gas_desc%concs(idx_gas)%conc(1,1)  - input_minvals(igas)) / &
                    & (input_maxvals(igas) - input_minvals(igas))
            end do
          end do
          ...
          do icol = 1, ncol
            do ilay = 1, nlay
                nn_inputs(igas,ilay,icol)    =  (gas_desc%concs(idx_gas)%conc(ilay,1)  - input_minvals(igas)) &
                    & / (input_maxvals(igas) - input_minvals(igas))
            end do
          end do
        else 
          ...
          do icol = 1, ncol
            do ilay = 1, nlay
                nn_inputs(igas,ilay,icol)    =  (gas_desc%concs(idx_gas)%conc(ilay,icol)  - input_minvals(igas)) & 
                    & / (input_maxvals(igas) - input_minvals(igas))
            end do
          end do
        end if 
  ...
  function compute_gas_optics_ecrad(this,                        &
                            ...
                            tau,  ssa,                        &
                            tlev, tsfc, sfc_source, lev_source) result(error_msg)
    ...
    neta = size(this%kmajor,dim=2)
    npres = size(this%kmajor,dim=3)-1
    ntemp = size(this%kmajor,dim=4)
    ...
    ret =  gptlstart('compute_tau_ray_kernel')
      ...
      call compute_tau_rayleigh(         & !Rayleigh scattering optical depths
            ...
            ngas,nflav,neta,ntemp, & ! dimensions
            this%gpoint_flavor,          &
            this%get_band_lims_gpoint(), &
  ...
  function init_abs_coeffs(this, &
    ...
    integer :: ngas
    ! --------------------------------------
    err_message = this%init(band_lims_wavenum, band2gpt)
    ...
    do i = 1, ngas
      ...
      gas_is_present(i) = string_in_array(gas_names(i), available_gases)
    end do
    !
