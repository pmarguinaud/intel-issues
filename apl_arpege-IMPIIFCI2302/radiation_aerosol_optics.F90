...
#include "radiation_delta_eddington.h"
  ...
  subroutine add_aerosol_optics(ng_sw_in, ng_lw_in, nbnd_sw_in, nbnd_lw_in, &
...
#define nbnd_lw nbnd_lw_in
    ...
    ! spectrum
    real(jprb), dimension(nbnd_sw,nlev,istartcol:iendcol) &
         &  :: od_sw_aerosol, scat_sw_aerosol, scat_g_sw_aerosol
    real(jprb), dimension(nbnd_lw,nlev,istartcol:iendcol) &
         &  :: od_lw_aerosol
    ...
    if (aerosol%is_direct) then
      ...
      do jcol = istartcol,iendcol
        ...
        if (.not. config%do_sw_delta_scaling_with_gases) then
          ...
          ! the solver
          call delta_eddington_extensive_vec(nbnd_sw*nlev, od_sw_aerosol(:,:,jcol), &
        ...
        if (config%do_lw_aerosol_scattering) then
          ...
          call delta_eddington_extensive_vec(nbnd_lw*nlev, od_lw_aerosol(:,:,jcol), &
